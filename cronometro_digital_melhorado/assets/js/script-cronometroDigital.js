...
// Selecionando os elementos de configuração do som:
var statusSom = document.getElementById('status-som');
var intervaloSom = document.getElementById("intervalo-som");
var duracaoSom = document.getElementById('duracao-som');

console.log(intervaloSom);

for(let i=2; i <= 59; i++){

    let opcaoIntervalo = document.createElement('option');
    opcaoIntervalo.innerText = `${i} minutos`;
    opcaoIntervalo.setAttribute('value',i);
    intervaloSom.appendChild(opcaoIntervalo);

    let opcaoDuracao   = document.createElement("option");
    opcaoDuracao.innerText = `${i} segundos`;
    opcaoDuracao.setAttribute('value', i);
    duracaoSom.appendChild(opcaoDuracao);    
}
...
var loop = setInterval(function(){
    ...
    if(statusExecucao == "RUN"){
        ...
        // Verificação do status do som:
        if(statusSom.value == "on"){
            console.log(`Intervalado som: ${intervaloSom.value}`);
            // Configuração do aviso sonoro:

            if(minutos > 0 && minutos % intervaloSom.value == 0){
                if(segundos <= duracaoSom.value){
                    somAviso.play();
                }//endif inner1
            }//endif inner2
        }//endif outer
